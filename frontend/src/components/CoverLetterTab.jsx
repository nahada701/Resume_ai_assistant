...
export default function CoverLetterTab({ resumeText, parsedData }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="card">
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>Cover Letter Generator</div>
        <div style={{ color: '#718096', fontSize: 13, marginBottom: 16 }}>Generate a personalized, compelling cover letter tailored to the specific role and company.</div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#a0aec0', display: 'block', marginBottom: 6 }}>
              Your Full Name *
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#a0aec0', display: 'block', marginBottom: 6 }}>Company Name *</label>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea className="input" value={jobDesc} onChange={e => setJobDesc(e.target.value)}
            placeholder="Paste the full job description here..." style={{ minHeight: 130 }} />
        </div>
      ...
      {(text || loading) && (
        <div className="card fade-in">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontWeight: 600, color: '#7f9cf5' }}>Cover Letter</div>
            {text && (
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn-secondary" onClick={copy} style={{ padding: '6px 12px', fontSize: 12 }}>
                  <Copy size={13} /> Copy
                </button>
                <button className="btn-secondary" onClick={download} style={{ padding: '6px 12px', fontSize: 12 }}>
          ...
          <div style={{
            ...
            padding: 20, background: '#0f1117', borderRadius: 8, border: '1px solid #2d3748',
