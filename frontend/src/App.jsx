...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f1117' }}>
      <header style={{ borderBottom: '1px solid #1e2334', padding: '14px 0', position: 'sticky', top: 0, background: '#0f1117', zIndex: 10 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 800, fontSize: 18 }} className="gradient-text">Resume AI Assistant</div>
            <div style={{ fontSize: 11, color: '#4a5568' }}>Powered by LLaMA 3.3 · Groq</div>
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            {parsedData?.name && (
              <span style={{ fontSize: 12, color: '#a0aec0', background: '#1a1d27', border: '1px solid #2d3748', borderRadius: 6, padding: '4px 10px' }}>
                {parsedData.name}
              </span>
            )}
            {resumeText && (
      ...
      <main style={{
        ...
        {/* Sidebar */}
        <div style={{ position: isMobile ? 'static' : 'sticky', top: 78 }}>
          ...
          {parsedData && (
            <div className="card fade-in" style={{ marginTop: 14, padding: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#4a5568', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Parsed Info</div>

              {parsedData.name && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: '#4a5568', marginBottom: 3 }}>NAME</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0' }}>{parsedData.name}</div>
                </div>
              )}
              {parsedData.email && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: '#4a5568', marginBottom: 3 }}>EMAIL</div>
                  <div style={{ fontSize: 12, color: '#a0aec0', wordBreak: 'break-all' }}>{parsedData.email}</div>
                </div>
              )}
              {parsedData.skills?.length > 0 && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: '#4a5568', marginBottom: 6 }}>SKILLS</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {parsedData.skills.slice(0, 12).map((s, i) => (
                      <span key={i} className="badge badge-blue" style={{ fontSize: 11, padding: '2px 8px' }}>{s}</span>
                    ))}
                    {parsedData.skills.length > 12 && (
                      <span style={{ fontSize: 11, color: '#4a5568' }}>+{parsedData.skills.length - 12} more</span>
                    )}
                  </div>
                </div>
              )}
              {parsedData.experience?.length > 0 && (
                <div>
                  <div style={{ fontSize: 11, color: '#4a5568', marginBottom: 6 }}>EXPERIENCE</div>
          ...
          {resumeText && !parsedData && (
            <div className="card" style={{ marginTop: 14, padding: 14 }}>
              ...
              <div style={{ fontSize: 11, color: '#718096', lineHeight: 1.6, maxHeight: 180, overflow: 'hidden', position: 'relative' }}>
                {resumeText.slice(0, 350)}
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 40, background: 'linear-gradient(transparent, #1a1d27)' }} />
              </div>
            </div>
          )}
        </div>
        ...
        <div>
          {/* Tab bar */}
          <div style={{
            display: 'flex', gap: 3, background: '#1a1d27', padding: 4,
            borderRadius: 10, marginBottom: 20, border: '1px solid #2d3748',
          ...
          {activeTab === 'improve' && <ImproveTab resumeText={resumeText} />}
          {activeTab === 'cover' && <CoverLetterTab resumeText={resumeText} parsedData={parsedData} />}
          {activeTab === 'match' && <JobMatchTab resumeText={resumeText} isMobile={isMobile} />}
          {activeTab === 'linkedin' && <LinkedInTab resumeText={resumeText} />}
          {activeTab === 'bullets' && <BulletsTab resumeText={resumeText} />}
          {activeTab === 'chat' && <ChatTab resumeText={resumeText} />}
        </div>
      </main>

      <footer style={{ textAlign: 'center', padding: '28px 24px', color: '#2d3748', fontSize: 12, borderTop: '1px solid #1e2334', marginTop: 32 }}>
